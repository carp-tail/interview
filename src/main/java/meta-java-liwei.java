// 本试卷唯一 ID: 206B9DE14759FFB6C9BBF2BD7F7E015E, 请勿修改本行内容
/* 注意, 这里不要写包名. 留空 */
...
class MetaJavaQuestionSheet {
    ...
    */
    ...
    /* qn5:
        下面代码会分别输出怎样的结果:
        String s = new String(new char[] {'没','人','比','我','更','懂','j','a','v','a'});   堆中
        String si = "没人比我更懂java";  字符串常量池
    ...
    */
    ...
    public static Set<String> showMap(Map<String, Object> map) {
        ...
        putIntoSetMapHelper(sb, map);
        return set;
    }

    static void putIntoSetMapHelper(StringBuilder sb, Map<String,Object> map){
        if (sb.length() != 0){
            sb.append(".");
        }
        ...
        for (String key : map.keySet()){
            ...
            if (value == null || value instanceof String){
                sb.append(" = ").append((String) value); // 为空或字符串
            }else if (value instanceof Number){
                sb.append(" = " + (Number) ((Number) value).longValue()); // 为Number
            }else if (value instanceof Object[]){ // 数组的情况
                putIntoSetArrayHelper(sb, (Object[]) value);
            }
            set.add(new String(sb));
            sb = new StringBuilder(sb.subSequence(0 , length)); // 遍历内容之后的字符
        }
    }

    // 回溯
    static void putIntoSetArrayHelper(StringBuilder sb, Object[] objects){
        int length = sb.length(); // 当前长度
        for (int i = 0; i < objects.length; i++){
            sb.append("[" + i + "]");
            Object o = objects[i];
            if (o == null || o instanceof String){
                sb.append(" = ").append((String) o); // 为空或字符串
            }else if (o instanceof Number){
                sb.append(" = " + (Number) ((Number) o).longValue()); // 为Number
            }else if (o instanceof Object[]){ // 数组的情况
                putIntoSetArrayHelper(sb, (Object[]) o);
            }else if (o instanceof Map){
                putIntoSetMapHelper(sb,(Map<String,Object>) o);
            }
            set.add(new String(sb));
            sb = new StringBuilder(sb.subSequence(0 , length));; // 遍历内容之后的字符
        }
    ...
    static boolean isTreeSymmetricHelper(TreeNode node1, TreeNode node2){
        ...
        return node1.val == node2.val && isTreeSymmetricHelper(node1.left, node2.right) && isTreeSymmetricHelper(node1.right, node2.left);
    }
}
